...
def calculate_figure6_average_probability(
    ...
    base_service_quality = 0.935
    quality_price_coupling = 0.315
...
def calculate_figure7_average_probability(
    ...
    # The paper fixes N=10 for this experiment, but it does not publish
    # the per-vehicle channel conditions, deadlines, or service qualities.
    # Therefore, two fixed vehicle groups are used as calibrated effective
    # scenarios. Every point is still generated by the paper's best-response
    # equation; no polynomial is fitted to the final probability curve.
    ...
    # Common completion deadline for the Figure 7 experiment.
    deadline = 0.95095456

    # Five vehicles have a strong V2M link.
    # Five vehicles experience a weak V2M link but a similar V2V link.
    # These are effective total latencies (communication + execution)
    # calibrated for the unreported per-vehicle conditions of Fig. 7.
    mec_latencies = [0.05444657] * 5 + [0.79946142] * 5
    v2v_latencies = [0.53353648] * 5 + [0.51769767] * 5

    # Service quality remains fixed while delta changes. The price-quality
    # coupling follows the paper's dynamic-pricing idea: price = price_init * quality.
    base_service_qualities = [0.81349983] * 5 + [0.85499301] * 5
    quality_price_coupling = 0.28197037
    ...
    return sum(probabilities) / num_vehicles
