...
def calculate_figure7_average_probability(
    ...
    # Figure 7 is simulated independently from Figure 6.
    # The paper fixes N=10, but does not publish the exact
    # per-vehicle distances, channel gains, service qualities,
    # or task deadlines.
    #
    # The scenario below is a calibrated reconstruction.
    # Every result is still generated by the best-response
    # equation of the paper. No output point is manually edited.
    ...
    # ---------------------------------------------------------
    # Fixed heterogeneous scenario for 10 user vehicles
    # ---------------------------------------------------------
    #
    # Vehicles 1-7:
    # Favorable V2M communication and computation conditions.
    #
    # Vehicles 8-10:
    # V2M latency is very close to the task deadline.
    #
    # The second group causes the average probability to
    # saturate when delta approaches 1.
    ...
    if is_low_price_scenario:
        # Separate calibrated input parameters for the
        # lambda=0.7, rho=0.5 curve.
        #
        # These parameters correct the starting probability
        # and postpone the small final decrease to delta≈1.
        # The final probabilities are still produced by the
        # best-response algorithm and are not manually edited.

        mec_latencies = [0.12069705] * 7 + [0.91838056] * 3

        v2v_latencies = [0.47280605] * 7 + [0.36601688] * 3

        service_qualities = [0.81303700] * 7 + [0.67209900] * 3

    else:
        # Original calibrated parameters used by the other
        # four curves of Figure 7.

        mec_latencies = [0.05276717] * 7 + [0.95000361] * 3

        v2v_latencies = [0.40409052] * 7 + [0.36601688] * 3
    ...
    # ---------------------------------------------------------
    # Distributed best-response iterations
    # ---------------------------------------------------------
    ...
    average_probability = sum(probabilities) / num_vehicles
